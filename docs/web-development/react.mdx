...
```
まず、`useState`という仕組みを使って、`count`という状態を宣言しています。初期値は0です。  
次に、`return`の中で「今どう見えるべきか」を書いています。`{count}`の部分は、`count`の値が入ります。  
ボタンには`onClick`というイベントハンドラを設定していて、クリックされたときに`setCount(count + 1)`が呼ばれます。これによって`count`の値が更新され、画面も自動的に再計算されます。  

JavaScriptの例と比べると、React側は「値を更新する」ことだけに集中できています。  
`count`が変わると、表示(`<h1>`)は自動的に再計算されます。

:::info
このように、Reactは`useState`という機能を軸に話が進みます。配列の中の`count`が状態を表し、`setCount`が状態を更新するための関数です。
:::

:::tip
ちなみにあしたぼホームページでは、例えばコマ表の表示する日付を管理するためにReactの`useState`を使っています。  
左右の矢印を押すと`viewDate`という状態を「その直前の状態から±7日する」という更新が走っています。  
これ以外にもいっぱい使ってて、なんと111個のuseStateがあるらしい！(2026年3月現在)
:::
