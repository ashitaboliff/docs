...
<Figure
  src="https://docs-cdn.ashitabo.net/web-dev/4-1.gif" 
  caption="ボタンをクリックするとh1の要素が書き変わるJavaScriptの動作例" 
  alt="ボタンをクリックするとh1の要素が書き変わるJavaScriptの動作例"
/>

特にbuttonタグは`onclick`属性を持っていて、ここにJavaScriptの関数名を書くと、そのボタンがクリックされたときにその関数が実行されます。

## 4.4 もっと高度な機能
たとえば、`modalButton`というIDを持つボタンをクリックしたときに、`modal`というIDを持つ要素の表示/非表示を切り替える例を見てみましょう。
...
<html>
  ...
  <body>
    <div id="my-id">
      <h1 id="my-id-2">ようこそ、あしたぼホームページへ！</h1>
      <p class="my-class">これはサンプルのHTMLページです。</p>
      <button id="modalButton">モーダルを開く/閉じる</button>
...
<Figure
  src="https://docs-cdn.ashitabo.net/web-dev/4-2.gif" 
  caption="モーダルウィンドウのJavaScriptの動作例" 
  alt="モーダルウィンドウのJavaScriptの動作例"
/>

このコードでは、`addEventListener`メソッドを使用して、`modalButton`がクリックされたときに実行される関数を登録しています。関数内では、`modal`要素の現在の表示状態を確認し、表示/非表示を切り替えています。

:::tip
JavaScriptではクリックやドラッグなどのWebサイト上でのユーザーの操作を「**イベント**」と呼びます。`addEventListener`は特定のイベントが発生したときに実行される関数を登録するためのメソッドです。  
また[4.3 JavaScriptでDOMを操作する](./js#43-javascriptでdomを操作する)のように特定の要素でクリックのイベントが発生したときに実行される関数を登録するための`onclick`属性もありますが、ノーマルのJavaScriptでは`addEventListener`の方がより柔軟で推奨されます。
:::
