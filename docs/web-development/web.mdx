...
:::warning
この章はNext.jsやReactなどのフレームワークを学ぶ前に、Webの基本的な仕組みを理解するためのものです。  
知識としては少し難しいので、すべてを理解する必要はなく、斜め読みで構いません。
:::
...
3. ウェブサーバーは、見つけたリソースをHTTPレスポンスとしてウェブブラウザに返します。実際に返されるレスポンスはブラウザの開発者ツールのネットワークタブなどで確認できますが、例えば以下のような形です。
    ...
    ...
    ```
    これは`ashitabo.net/booking`のHTMLファイルの一部です。  
    ちょっと長すぎるし細かい部分は置いておいて、ウェブサーバから`Content-Type: text/html`のようにHTMLファイルが返されていることがわかります。
...
- `text/javascript`: JavaScriptファイル。Webページに動的な機能を追加します。
