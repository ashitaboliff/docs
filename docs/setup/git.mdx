...
1. **SSHキーの生成**  
   ...
   />
   :::tip
   パスフレーズの設定は可能ならしておくとよいと思います。僕は設定していません。  
   ...
   :::
2. **SSH公開キーのコピー**  
   以下のコマンドを実行して、SSH公開キーの内容を表示、全体を選択してコピーします。
    ```bash
...
### Q1. `ssh -T git@github.com`で`Permission denied (publickey)`が出る
以下を順に確認してください。
1. 鍵ファイルがあるか確認
   ```bash
   ls -la ~/.ssh
   ```
   `id_ed25519`と`id_ed25519.pub`が見えるか確認します。
2. SSHエージェントに鍵を追加
   ```bash
   eval "$(ssh-agent -s)"
   ssh-add ~/.ssh/id_ed25519
...
3. GitHubに公開鍵が登録されているか確認  
   GitHubの`Settings` -> `SSH and GPG keys`で、`id_ed25519.pub`の内容を貼り付けた鍵があるか確認します。
4. リモートURLがSSH形式か確認
   ```bash
   git remote -v
   ```
   `git@github.com:...`形式でなければ、HTTPS接続になっている可能性があります。

### Q2. `Please tell me who you are`でコミットできない
Gitのユーザー情報が未設定です。以下を実行してください。
```bash
git config --global user.name "あなたの名前"
git config --global user.email "あなたのメールアドレス"
```

### Q3. `Repository not found`でクローンできない
以下を確認してください。
1. 管理者からGitHub Organizationへの招待が来ているか  
2. 招待をGitHub上で承認したか  
3. クローン先URLにタイプミスがないか

### Q4. `WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED!`が出た
一度接続した`github.com`の公開鍵情報と現在の情報が一致しない状態です。  
むやみに進めず、管理者に確認してください。

以上でGitのセットアップは完了です。これでGitを使用してコードのバージョン管理やGitHubとの連携が可能になります。  
次に本格的にあしたぼホームページの開発環境をセットアップしていきましょう。
